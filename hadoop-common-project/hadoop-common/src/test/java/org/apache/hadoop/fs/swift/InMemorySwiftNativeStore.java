...
public class InMemorySwiftNativeStore extends SwiftNativeFileSystemStore {
  ...
  public void uploadFile(Path path, InputStream inputStream, long length) {
    ...
    metadataMap.put(path.toUri().toString(), new FileStatus(size, false, 0, 0, Time.now(), path));
  ...
  public void createDirectory(Path path) {
    metadataMap.put(path.toUri().toString(), new FileStatus(0, false, 0, 0, Time.now(), path));
  }

  @Override
  public List<URI> getObjectLocation(Path path) {
    return super.getObjectLocation(path);    //To change body of overridden methods use File | Settings | File Templates.
  }
